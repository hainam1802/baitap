...
@section('content')
@if(isset($data))
{{Form::open(array('route'=>array('admin.product.update',$data->id),'method'=>'PUT','id'=>'formMain','enctype'=>"multipart/form-data" , 'files' => true))}}
@else
{{Form::open(array('route'=>array('admin.product.store'),'method'=>'POST','id'=>'formMain','enctype'=>"multipart/form-data"))}}
@endif
<div class="row">
   <div class="col-lg-9">
         ...
         <div class="card-body">
            ...
            <div class="form-group row">
                <div class="col-6 col-md-6">
                    <label for="name">{{ __('Tên phòng')}} <span style="color: red">(*)</span></label>
                    <input type="text" name="title"
                    value="{{ old('title', isset($data) ? $data->title : null) }}"
                    placeholder="{{ __('Tên phòng') }}"  autocomplete="off" id="title_gen_slug"
                ...
                <div class="col-6 col-md-6">
                    <label for="quantity_people">{{ __('Số lượng người')}} <span style="color: red">(*)</span></label>
                    <input type="number" name="quantity_people"
                    value="{{ old('quantity_people', isset($data) ? $data->quantity_people : null) }}"
                    placeholder="{{ __('Số lượng người') }}"  autocomplete="off"
                    class="form-control {{ $errors->has('quantity_people') ? ' is-invalid' : '' }}">
                    @if ($errors->has('quantity_people'))
                    <span class="form-text text-danger">{{ $errors->first('quantity_people') }}</span>
                    @endif
                 </div>
                <div class="col-6 col-md-6 mt-4">
                    <label for="price">{{ __('Giá phòng')}} <span style="color: red">(*)</span></label>
                    <input type="text" name="price"
                    value="{{ old('price', isset($data) ? $data->price : null) }}"
                    placeholder="{{ __('Giá') }}"  autocomplete="off"
                ...
                <div class="col-6 col-md-6 mt-4">
                    <label for="price">{{ __('Giá cũ')}} </label>
                    <input type="number" name="price_old"
                    value="{{ old('price_old', isset($data) ? $data->price_old : null) }}"
                    placeholder="{{ __('Giá cũ') }}"  autocomplete="off"
                ...
                {{-- khu ở --}}
                <div class="col-6 col-md-6 mt-4">
                    <label for="category_id" class="form-control-label">{{ __('Khu ở') }} <span style="color: red">(*)</span></label>
                    <select name="category_id" class="form-control " style="width: 100%">
                        <option value="0">-- {{__('Không chọn')}} --</option>
                        @if( !empty(old('category_id')) )
                            {!!\App\Library\Helpers::buildMenuDropdownList($dataCategory,old('category_id')) !!}
                        @else
                            {!!\App\Library\Helpers::buildMenuDropdownList($dataCategory,$data->category_id??'') !!}
                ...
                {{-- loại phòng --}}
                <div class="col-6 col-md-6 mt-4">
                    <label for="group_id" class="form-control-label">{{ __('Loại phòng') }} <span style="color: red">(*)</span></label>
                    <select name="group_id" class="form-control" style="width: 100%">
                        <option value="0">-- {{__('Không chọn')}} --</option>
                        @if( !empty(old('group_id')) )
                            {!!\App\Library\Helpers::buildMenuDropdownList($dataGroups,old('group_id')) !!}
                        @else

                            {!!\App\Library\Helpers::buildMenuDropdownList($dataGroups,$data->group_id??'') !!}
                ...
                {{-- Địa điểm --}}
                <div class="col-6 col-md-6 mt-4">
                    <label for="locale_id" class="form-control-label">{{ __('Địa điểm') }} <span style="color: red">(*)</span></label>
                    <select name="locale_id" class="form-control" style="width: 100%">
                        <option value="0">-- {{__('Không chọn')}} --</option>
                        @if( !empty(old('locale_id')) )
                            {!!\App\Library\Helpers::buildMenuDropdownList($dataLocale,old('locale_id')) !!}
                        @else
                            {!!\App\Library\Helpers::buildMenuDropdownList($dataLocale,$data->locale_id??'') !!}
                    ...
                    @if($errors->has('locale_id'))
                        <div class="form-control-feedback">{{ $errors->first('locale_id') }}</div>
                    @endif

                </div>
                <div class="col-6 col-md-6 mt-4">
                    <label for="quantity">{{ __('Số lượng phòng')}} <span style="color: red">(*)</span></label>
                    <input type="number" name="quantity"
                           value="{{ old('quantity', isset($data) ? $data->quantity : null) }}"
                           placeholder="{{ __('Số lượng phòng') }}"  autocomplete="off"
...
{{--                    {{Form::select('type',config('module.product.type'),old('type', isset($data) ? $data->type : null),array('class'=>'form-control'))}}--}}
{{--                    @if($errors->has('type'))--}}
             ...
             <div class="form-group row">
                 ...
                 <div class="col-md-12">
                     ...
                     @if (isset($data) && isset($data->image_detail) && $data->image_detail != null)
                         ...
                         <br>
                     @endif
                     <input id="input-b3" name="image_detail[]" type="file" style="color: black" class="file" multiple data-show-upload="false" data-show-caption="true" data-msg-placeholder="Chọn ảnh">
                 </div>
{{--                 --}}{{-----icon------}}
{{--                 <div class="col-md-4">--}}
